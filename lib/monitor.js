...
monitor.setConfig = function(configuration){
  ...
  this.env = this.config.dev_env? 'development' : 'production';
    ...
    stats.forEach(function(stat){
      ...
      switch(stat.type){
        ...
        case 'timer':
          ...
          self.sendMetricToDDog("gauge", stat.key, stat.value);
          break;
      }
...
monitor.sendMetricToDDog = function(type, key, value) {
  ...
  this.ddog.postSeries(counters, function(error, response, body) {
    if(!response){
      return winston.log("warn", util.format("Unexpected Code [%s] while sending stat [ %s => %s, %s] to DDog.", null, os.hostname(), key, value));
    }
